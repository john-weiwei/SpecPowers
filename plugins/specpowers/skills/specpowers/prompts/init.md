# init.md — 项目原则生成契约（init 阶段，产物仍为 constitution.md）

## 触发

当用户调用 `/specpowers-init [--force]` 时加载本契约。

## 前置校验

1. 确认当前目录是 git 仓库
2. 运行 `python -m specpowers_cli.bridge.facade init [--force] --root .` 完成状态机校验
3. 检查返回：若 stage 非 init 且无 --force → 提示用户确认（会丢弃当前 feature 流水线）
...
- `/specpowers-explore "<需求>"` — 启动特性探索
- `/specpowers-propose "<需求>"` — 直接进入提案（一站式生成三件套）
