...
```bash
python bridge/facade.py fast "<需求>" --root .
```
...
Dispatcher 基于需求+对话生成 **3-5 条验收清单**：
...
优化模式 archive 退化：
- **不运行** OpenSpec delta 合并
- `git commit -m "specpowers(fast): <feature> — <改动摘要>"`
- 写 OpenSpec stub（模式标记 + commit hash + 改动范围）
